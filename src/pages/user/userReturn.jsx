...
export default function UserBorrow() {
  ...
  if (borrows.length === 0) {
    return <p className="text-center py-4 text-gray-100 bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500">No borrow records found.</p>;
  }
          ...
          {borrows.map((borrow) => {
            ...
            return (
              <tr key={borrow.borrowId} className="hover:bg-gray-100">
                ...
                <td className="border border-gray-400 px-6 py-4">{new Date(borrow.borrowDate).toLocaleDateString()}</td>
                <td className="border border-gray-400 px-6 py-4">{new Date(borrow.dueDate).toLocaleDateString()}</td>
                ...
                <td className="border border-gray-400 px-6 py-4">
                  <span className={`px-2 py-1 rounded text-xs font-semibold ${isReturned ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"}`}>
